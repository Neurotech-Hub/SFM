...
templates, and behavior reports under `packages/dev_gui/`.
...
live in [HARDCODED_VALUES.md](../../../firmware/docs/HARDCODED_VALUES.md). GUI-exposed experiment
...
| [1, 2, 3] | `kit.session` default nodes | `kit.py` | Used when `build()` is called with `nodes=None`. GUI always passes the live roster |
...
These are the JSON form defaults (`packages/dev_gui/experiments/*.json`). Changing
the JSON changes what the GUI Start form shows; Python `build()` kwargs are the
fallbacks (kept in sync).
...
[`packages/sfm-analysis`](../../sfm-analysis/README.md) — every
