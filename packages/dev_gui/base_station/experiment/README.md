...
### Session naming and the unified log
...
analysis — this is how a session picks back up across GUI restarts.
...
A GUI-hosted template is **two files** with the same `<name>`. Dropping them
in these locations is enough — `schema.resolve_builder()` imports
`base_station.experiment.templates.<name>.build` dynamically; there is no
central registry to edit.

```
packages/dev_gui/
├── experiments/
│   └── <name>.json                         # GUI form (parameters, types, defaults)
└── base_station/experiment/
    ├── README.md                           # this guide
    ├── schema.py                           # loads every experiments/*.json
    ├── kit.py                              # optional helpers (session, advance, faults)
    ├── templates/
    │   ├── __init__.py                     # optional re-export only
    │   └── <name>.py                       # def build(**kwargs) -> Experiment
    └── ...                                 # engine (events, context, script, runner)
```

Copy-from starters that already follow this layout (event-based round-robin,
and an `@exp.script` BNC-paced FR1):
`[examples/templates/](../../examples/templates/)`. Copy the `.py` into
`templates/` and the `.json` into `experiments/`, then relaunch the GUI.

Optional extras, same `<name>`:

```
packages/dev_gui/tests/test_<name>.py                    # make_runner() + inject()
packages/sfm-analysis/src/sfm_analysis/report/
├── designs/<name>.json                                  # which report sections to render
├── analyses/<name>.py                                   # numbers from the log
└── sections/<name>.py                                   # HTML for those numbers
```

The report side is optional — without it, `sfm-report` falls back to
`default.json`. Starter files:
...
Two complete, commented templates live in
[`examples/templates/`](../../examples/templates/) — they are **not**
auto-loaded. Copy each pair into `experiments/<name>.json` and
`templates/<name>.py` to register them; they appear in the GUI on the
next launch.


| Starter       | Style                                                  | What it shows                                                                                                              |
| ------------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `alternation` | Event-based (`@exp.on_start` / `@exp.on_pellet_taken`) | Round-robin reload, `control.after`, `control.next_trial()`, custom EXP rows (`alternation_start` / `alternation_advance`) |
| `bnc_paced`   | Sequential (`@exp.script`)                             | `yield control.wait_for("bnc_in")` then `"pellet_taken"`, `.faulted` handling, a fixed ITI                                 |


Event-based skeleton (the `alternation` idea, shortened):

```python
from base_station.experiment.kit import log_faults, session

def build(nodes=None, *, delay_s=2.0, minutes=0, max_pellets=None, **_):
    exp = session("alternation", nodes, minutes=minutes, max_pellets=max_pellets)
    log_faults(exp)
    ...
    def _advance(control, event):
        ...
        nxt = roster[(roster.index(event.node_id) + 1) % len(roster)]
        def _go():
            control.next_trial()
            control.dispense(nxt)
        control.after(delay_s, _go, node=nxt)

    return exp
```

`@exp.script` skeleton (the `bnc_paced` idea, shortened):
...
def run(control):
    ...
    while True:
        ...
        if not edge.ok or (edge.event.data or {}).get("edge") != "rising":
            continue
        control.next_trial()
        control.dispense(node)
...
per-node sticky fault + recover. (`reload_delay_s=` is a legacy alias
of `fixed_delay_s`.)
...
(a comma-separated string is also accepted). Same shared
...
the dispense motion (`control.dispense(fed)` / `control.dispense(empty, feed=False)`), but only one delivers, chosen by a reward probability that
...
Drive it with synthetic events — no CAN hardware and no GUI needed:
