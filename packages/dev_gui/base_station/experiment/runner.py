...
Users build an Experiment (decorators + start/end conditions). The GUI
hosts it via ``make_runner()`` / ``runner.step(now)`` each frame.
Tests use the same runner with no CAN bound and ``inject()`` / ``step()``.
"""
...
class Experiment:
    ...
    Example::
        ...
        # Hosted by the GUI Experiment panel, or in tests:
        # runner = exp.make_runner(); runner.start(now=0.0); runner.step(now=1.0)
    """
...
class ExperimentRunner:
    ...
    Call ``step(now)`` each frame when hosted by the DearPyGui app.
    Tests call ``inject()`` / ``step()`` with no CAN bound.
    """
